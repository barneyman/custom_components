...
class HeadCountSensor(Entity):
    ...
    def __init__(self, hass):
        ...
        async def _async_sensor_changed(entity_id, old_state, new_state):
            if new_state is None:
                return
            _LOGGER.info("seen state change for {}".format(entity_id))

            result = await hass.async_add_executor_job(self.update)
        ...
        for entity_id in hass.states.entity_ids('device_tracker'):
            _LOGGER.info("subscribing to state change for {}".format(entity_id))
    ...
    def update(self):
        ...
        _LOGGER.info("headcount {}".format(home))
